...
fn count_free_in_row(sensors: &Vec<SensorBeaconPair>, row: i32) -> usize {
    ...
    let mut blockedCells = HashSet::new();

    for pair in sensors {
        let r = pair.distance() as i32;
        let rowDst = (row - pair.sensor.y).abs();
        ...
        if rowDst <= r {
            ...
            for i in -w..w + 1 {
                ...
                blockedCells.insert(pair.sensor.x + i);
            }
        }
    }

    return blockedCells.len() as usize;
}
...
fn find_signal(data: &Vec<SensorBeaconPair>, area: i32) -> Position {
    for pair in data {
        for p in pattern_from_center(&pair.sensor, (pair.distance() + 1) as i32, area) {
            if data.iter().any(|n| n.sensor.distance(&p) <= n.sensor.distance(&n.beacon)) == false {
                return p;
            }
        }
    }
...
impl Day for Day15 {
    ...
    fn run(&self) {
        ...
        println!("Part two: {}", pos.x as u64 * 4000000 as u64 + pos.y as u64);
    }
}