...
impl Cave {
    ...
    fn from_points(points: &Vec<Vec<Position>>) -> Self {
        ...
        let mut rocks = vec![
            vec![Material::Air; (width - width_min + 1) as usize]; (height - height_min + 1) as usize
        ];
        ...
        rocks[sand_source_local.y as usize][sand_source_local.x as usize] = Material::Sand_Source;
        ...
        for point_vec in points {
            for i in 0..point_vec.len() - 1 {
                ...
                for x in lx..=ux {
                    for y in ly..=uy {
                        rocks[y as usize][x as usize] = Material::Rock;
                    }
                }
            }
        }
        ...
        return Cave {
            rocks,
            width: (width - width_min + 1) as usize,
            height: (height - height_min + 1) as usize,
            width_min,
            unit: 0
        }
    }
...
impl Position {
    fn from_str(s: &str) -> Vec<Self> {
        return s.split(" -> ").into_iter().map(|coord| {
